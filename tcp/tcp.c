...
*/
		...
		/* this function supports both mode
		 * if following return statement is commented, 
		 * then tcp_write will assure thate all data given to it is sent
		 * if following return statement is not commented,
		 * then tcp write will send only that much data which can fit into 
		 * one packet, and then return the bytes_sent to calling function */
		return bytes_sent ;		
...
write_packet (char * buf, int len, int flags )
{
	...
	Data dat ;
	int bytes_sent ;
	u32_t ack_to_wait ; /* what ack_no to wait ? */
