...
recv_tcp_packet(Header* hdr, Data* dat)
{
    ...
    /*  This memory is to be freed by calling function. In this case,
        it is handle_packets()
     */
...
tcp_socket()
{
    ...
    /* FIXME :need to create linked list */
...
tcp_connect (ipaddr_t dst, int port )
{
	...
	/* FIXME : better way to assign local port (allocation) */
	Head->sport = 6000;
	cc->state = Syn_Sent;
...
tcp_read(char *buf, int maxlen)
{
	...
	if (remaining_bytes > 0) {
		memmove(cc->in_buffer->content, (cc->in_buffer->content+bytes_read),
                remaining_bytes);
	}
...
tcp_write(char * buf, int len)
{
	...
	while (bytes_left > 0) {
		...
		dprint("tcp_write: writing %d bytes of packet by calling
                 write_packet\n", packet_size);
...
tcp_close(void)
{
	...
	switch (cc->state) {
        case Fin_Wait1:
            ...
            break;
		case Closing:
            dprint("tcp_close: Closing: Fin Acknowledged, going to
                        Time_Wait\n");
            cc->state = Time_Wait;
			break;
		case Last_Ack:
...
handle_Syn_Sent_state(Header *hdr, Data *dat)
{
    ...
    if (!(flags & SYN)) {
		/* Didn't get expected packet...
            ...
            hence, not changing the state
         */
		dprint("handle_Syn_Sent_state: Got non-SYN packet, ignoring it\n");
		return -1;
	}
...
handle_Syn_Recv_state(Header *hdr, Data *dat)
{
	...
	if (!(flags & ACK )) {
		/* Didn't get expected packet...
            ...
            hence, not changing the state
        */
        dprint("handle_Syn_Recv_state: didn't get ACK,
                so ignoring packet\n");
		return -1;
	}
	
	/* FIXME: you may get FIN packet at this stage...
	   in which case need to change to FIN_WAIT_1 state
	 */
