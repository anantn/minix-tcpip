...
tcp_listen(int port, ipaddr_t * src)
{
	...
	/*
	 * FIXME : port varification is not done yet.. accepting packet for
	 * any port
	 */
	do {
		handle_packets();
...
tcp_write(char *buf, int len)
{
	...
	while (bytes_left > 0) {
		...
		/*
		 * FIXME:upgrade the code for supporting multiple writes to
		 * send all data
		 */
		return bytes_sent;
...
write_packet(char *buf, int len, int flags)
{
	...
	Data            dat;
	int             bytes_sent;
	int             ack_to_wait;	/* what ack_no to wait ? */
	...
	cc->local_seqno = cc->local_seqno + len;	/* FIXME : handle
							 * overflowing 	 */
	...
	if (flags & FIN) {
		...
		if (len == 0) {
			 ...
			 */
			/*
			 * cc->local_seqno = cc->local_seqno + 1 ; *//* FIXME
			 * : handle overflowing
			 */
			ack_to_wait = cc->local_seqno + 1;
		}
...
wait_for_ack(u32_t local_seqno)
{
	...
	while (local_seqno > cc->remote_ackno) {	/* FIXME :need to worry
							 * about overflowing in
							 * comparision */
	 ...
	 * assume that last packet which is present in cc->unack_header is
	 * acked, so delete it. and cancel the alarm which was set.
	 */
	/*
	 * may be i should get a lock before i modify these thing, that will
	 * assure the correctness better locking can be implemented by
	 * masking the signal temporarily
	 */
...
setup_packet(Header * hdr)
{
	...
	cc = Head->this;
	/*
	 * hdr->src = 	// set at IP layer hdr->prot = // set at ip layer
...
handle_Listen_state(Header * hdr, Data * dat)
{
	...
	cc->remote_seqno = hdr->seqno + 1;	/* FIXME :need to worry about
						 * overflowing */
	cc->remote_window = hdr->window;	/* updating remote window
						 * size */
	cc->state = Syn_Recv;
...
handle_Syn_Sent_state(Header * hdr, Data * dat)
{
	...
	if (flags & ACK) {
		...
		if (cc->local_seqno + 1 != hdr->ackno) {	/* FIXME :need to worry
								 * about overflowing */
		...
		cc->remote_seqno = hdr->seqno + 1;	/* FIXME :need to worry
							 * about overflowing */
		cc->remote_window = hdr->window;	/* updating remote
							 * window size* */
...
handle_Syn_Recv_state(Header * hdr, Data * dat)
{
	...
	/*
	 * FIXME : you may get FIN packet at this stage... then u need to
	...
	cc->remote_ackno = hdr->ackno;
	cc->local_seqno = hdr->ackno;
	cc->remote_seqno = hdr->seqno + dat->len;	/* FIXME :need to worry
							 * about overflowing */
	cc->remote_window = hdr->window;	/* updating remote window
						 * size */
	/* need to send ack for this */
	...
	if ((dat->len > 0) || (cc->in_buffer->len + dat->len <= DATA_SIZE)) {
		...
		/*
		 * there is data in this packet, and also there is space in
		 * buffer so, put this data in buffer
		 */
		dprint("Length: %d\n", dat->len);
...
handle_Established_state(Header * hdr, Data * dat)
{
	...
	if ((dat->len == 0) || (cc->in_buffer->len + dat->len <= DATA_SIZE)) {
		...
		if (dat->len != 0) {
			...
			if (hdr->flags & FIN) {
				...
				/*
				 * cc->remote_seqno = hdr->seqno + 1 ; *//* FI
				 * XME :need to worry about overflowing
				 */
				cc->remote_seqno = hdr->seqno;
	...
	} else {		/* no buffer to store data, discard the
